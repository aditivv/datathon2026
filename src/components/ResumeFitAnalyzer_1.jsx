...
function EmailResults({ verdict }) {
  ...
  const handleSend = async () => {
    ...
    setStatus("sending");
    try {
      const res = await fetch("http://localhost:5001/send-results", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ email, verdict }),
      });
      ...
      {status === "sent" ? (
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{
            ...
            display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, color: "#00E5A0"
          }}>✓</div>
          <div>
            <div style={{ fontSize: 14, fontWeight: 500, color: "#00E5A0", marginBottom: 2 }}>Results sent!</div>
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.35)" }}>Check your inbox at <span style={{ color: "rgba(255,255,255,0.55)" }}>{email}</span></div>
          </div>
        </div>
      ) : (
        <>
          <div style={{ marginBottom: 14 }}>
            ...
            <div style={{ fontSize: 12, color: "rgba(255,255,255,0.3)" }}>
              Get a copy of your fit score and breakdown in your inbox.
            </div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <input
              ...
              style={{
                ...
                border: `1px solid ${status === "error" ? "rgba(255,77,106,0.4)" : isValid && email ? "rgba(99,102,241,0.4)" : "rgba(255,255,255,0.1)"}`,
                borderRadius: 10, color: "#fff", fontSize: 13,
                fontFamily: "'DM Sans', sans-serif", outline: "none", transition: "border-color 0.2s"
              }}
            />
            <button
              ...
              style={{
                padding: "11px 22px", borderRadius: 10, fontSize: 12, fontWeight: 600,
                fontFamily: "'Space Mono', monospace", letterSpacing: "0.06em",
                cursor: isValid && status !== "sending" ? "pointer" : "not-allowed",
                background: isValid ? "rgba(99,102,241,0.2)" : "rgba(255,255,255,0.04)",
                border: `1px solid ${isValid ? "rgba(99,102,241,0.4)" : "rgba(255,255,255,0.08)"}`,
                color: isValid ? "#A78BFA" : "rgba(255,255,255,0.2)",
                transition: "all 0.2s", whiteSpace: "nowrap",
          ...
          {status === "error" && (
            <div style={{ marginTop: 8, fontSize: 11, color: "#FF4D6A", fontFamily: "'Space Mono', monospace" }}>
              Failed to send — make sure the server is running on port 5001.
            </div>
          )}
        </>
      )}
